    ...
    it('should render visit booked page - FHI', () => {
      return request(app)
        ...
        .expect(res => {
          ...
          expect($('[data-test="prison-name"]').text()).toBe('Foston Hall (HMP)')
          expect($('[data-test="prison-number"]').text()).toBe('01234567890')
    ...
    it('should render visit booked page - DHI', () => {
      ...
      return request(app)
        ...
        .expect(res => {
          ...
          expect($('[data-test="phone-number-text"]').length).toBe(1)
          expect($('[data-test="prison-specific-content"]').length).toBe(1)
          expect($('[data-test="prison-name"]').text()).toBe('Drake Hall (HMP)')
          expect($('[data-test="prison-number"]').text()).toBe('09876543210')
          expect($('[data-test="minutes-before-visit"]').text()).toBe('45')
