import { RestClient, SanitisedError, asUser } from '@ministryofjustice/hmpps-rest-client'
...
export default class OrchestrationApiClient {
  ...
  constructor(private readonly token: string) {
    this.restClient = new RestClient('orchestrationApiClient', config.apis.orchestration, logger)
  }
  ...
  async getFuturePublicVisits(bookerReference: string): Promise<OrchestrationVisitDto[]> {
    return this.restClient.get({ path: `/public/booker/${bookerReference}/visits/booked/future` }, asUser(this.token))
  }

  async getPastPublicVisits(bookerReference: string): Promise<OrchestrationVisitDto[]> {
    return this.restClient.get({ path: `/public/booker/${bookerReference}/visits/booked/past` }, asUser(this.token))
  }

  async getCancelledPublicVisits(bookerReference: string): Promise<OrchestrationVisitDto[]> {
    return this.restClient.get({ path: `/public/booker/${bookerReference}/visits/cancelled` }, asUser(this.token))
  }
  ...
  async getPrisoners(bookerReference: string): Promise<BookerPrisonerInfoDto[]> {
    return this.restClient.get({ path: `/public/booker/${bookerReference}/permitted/prisoners` }, asUser(this.token))
  }
  ...
  async getSupportedPrisonIds(): Promise<string[]> {
    return this.restClient.get({ path: '/config/prisons/user-type/PUBLIC/supported' }, asUser(this.token))
  }

  async getPrison(prisonCode: string): Promise<PrisonDto> {
    return this.restClient.get({ path: `/config/prisons/prison/${prisonCode}` }, asUser(this.token))
  }
}
