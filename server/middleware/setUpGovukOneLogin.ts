...
export default function setUpGovukOneLogin(): Router {
  govukOneLogin.init().then(client => {
    ...
    router.get('/sign-in', (req, res, next) => {
      passport.authenticate('oidc', { nonce: generators.nonce(), state: generators.state() })(req, res, next)
    })

    router.get('/auth/callback', (req, res, next) => {
      passport.authenticate('oidc', { successRedirect: '/', failureRedirect: '/autherror' })(req, res, next)
    })
